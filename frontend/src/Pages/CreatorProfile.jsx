...
function CreatorProfile() {
  ...
  const fetchCreatorData = async () => {
    ...
    try {
      const [creatorRes, filesRes] = await Promise.all([
        ...
        api.get(`/files/user/${userId}`).catch(err => {
          console.error("Creator files fetch failed:", err);
          toast.error("Failed to load creator files");
  ...
  if (loading) {
    return (
      <div style={{ background: bg, minHeight: "100vh", color: "#e2e8f0", display: "flex", alignItems: "center", justifyContent: "center" }}>
    ...
    return (
      <div style={{ background: bg, minHeight: "100vh", color: "#e2e8f0", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className="text-center">
          <h1 className="text-2xl font-bold" style={gradText}>Creator Not Found</h1>
          <p style={{ color: "#64748b", marginTop: "1rem" }}>The creator you're looking for doesn't exist.</p>
      ...
      <div className="relative" style={{ zIndex: 1 }}>
        ...
        <div className="px-6 md:px-20">
          <div className="flex flex-col md:flex-row md:items-end gap-5 -mt-14 md:-mt-16 mb-8">
            <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden shrink-0" style={{ border: "3px solid #1a1a3e", boxShadow: "0 8px 32px #00000050" }}>
              <img src={userImg} alt={creator.username} className="w-full h-full object-cover" />
            </div>

            <div className="flex-1 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold" style={gradText}>{creator.username}</h1>
          ...
          <div className="flex flex-col lg:flex-row gap-8 mb-10">
            <div className="flex-1 flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2 text-xs" style={{ color: "#64748b" }}>
                ...
                <span className="flex items-center gap-1"><Calendar size={12} />Joined {new Date(creator.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
              ...
              {filtered.map((file) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <p className="font-semibold text-sm leading-snug" style={{ color: "#e2e8f0" }}>
                        {file.document?.originalName || "Unnamed File"}
                      </p>
                      <p className="text-xs mt-1" style={{ color: "#475569" }}>
                        {((file.document?.fileSize || 0) / 1024 / 1024).toFixed(1)} MB · {new Date(file.createdAt).toLocaleDateString()}
                      </p>
                    </div>

                    <div className="flex items-center justify-between text-xs" style={{ color: "#64748b" }}>
                      <span className="flex items-center gap-1"><Eye size={12} />0 views</span>
                      <span className="flex items-center gap-1"><Download size={12} />0 dl</span>
