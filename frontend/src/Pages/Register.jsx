...
import { User, Mail, Lock, UserPlus, Eye, EyeOff, CloudUpload, CheckCircle2 } from "lucide-react";
...
function InputField({ icon: Icon, type, name, placeholder, value, onChange, autoComplete }) {
  ...
  return (
    ...
    >
      ...
      />
      {isPassword && (
        <button type="button" onClick={() => setShowPw((v) => !v)} style={{ color: "#475569" }} className="hover:text-indigo-400 transition-colors">
...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password.length < 6) {
      toast.error("Password must be at least 6 characters", { style: toastStyle(false) });
      return;
    }
    setLoading(true);
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Registration failed", { style: toastStyle(false) });
      }
    } catch (err) {
      toast.error(
  ...
  return (
    ...
    >
      {/* ambient orbs */}
      <div style={{ position: "fixed", top: "-10%", right: "-8%", width: "500px", height: "500px", borderRadius: "50%", background: "radial-gradient(circle, #6366f116 0%, transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "fixed", bottom: "-8%", left: "-5%", width: "400px", height: "400px", borderRadius: "50%", background: "radial-gradient(circle, #8b5cf612 0%, transparent 70%)", pointerEvents: "none" }} />
      ...
      <div className="relative w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          <div>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", boxShadow: "0 6px 20px #6366f135" }}>
                <CloudUpload size={18} style={{ color: "#fff" }} />
              </div>
              <span className="text-xl font-bold" style={gradText}>Resare</span>
            </div>
          ...
          <div className="flex flex-col gap-3">
            {PERKS.map((perk) => (
              <div key={perk} className="flex items-center gap-3">
                <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0" style={{ background: "#4ade8015" }}>
        ...
        >
          ...
          <div className="flex flex-col gap-1">
            <h1 className="text-2xl font-bold" style={gradText}>Create account</h1>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium" style={{ color: "#64748b" }}>Username</label>
              <InputField icon={User} type="text" name="name" placeholder="coolcreator" value={formData.name} onChange={handleChange} autoComplete="username" />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium" style={{ color: "#64748b" }}>Email</label>
              <InputField icon={Mail} type="email" name="email" placeholder="you@example.com" value={formData.email} onChange={handleChange} autoComplete="email" />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium" style={{ color: "#64748b" }}>Password</label>
              <InputField icon={Lock} type="password" name="password" placeholder="Min. 6 characters" value={formData.password} onChange={handleChange} autoComplete="new-password" />
              {/* strength bar */}
                      ...
                      style={{
                        background:
                          formData.password.length >= i * 3
                            ? i <= 1 ? "#f87171" : i <= 2 ? "#fbbf24" : i <= 3 ? "#34d399" : "#4ade80"
                            : "#ffffff10",
                      }}
                    />
                  ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs" style={{ color: "#334155" }}>Already have an account?</span>
            <div className="flex-1 h-px" style={{ background: "#ffffff10" }} />
          </div>
