    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        const [filesData, profilesData] = await Promise.all([
          api
            .get(`/api/files?q=${normalizedQuery}`)
            .catch(() => ({ data: { files: [] } })),
          api
            .get(`/api/v1/auth/search?q=${normalizedQuery}`)
            .catch(() => ({ data: { users: [] } })),
        ]);
        ...
        if (filesResult.length === 0 && profilesResult.length === 0) {
          toast.error(`No results found for "${q}"`, { duration: 2000 });
        }
      } catch (err) {
  ...
  return (
    <div style={{ background: bg, minHeight: "100vh", color: "#e2e8f0" }}>
      ...
      >
        ...
        {loading ? (
          <div className="text-center mt-10" style={{ color: "#94a3b8" }}>
            Loading results...
          </div>
        ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs" style={{ color: "#475569" }}>
                        By {file.creatorId?.username || "Anonymous"}
                      </p>
                    </div>
                    <Link
                      to={`/file/${file._id}`}
